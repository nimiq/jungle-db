...
class LMDBBackend extends LMDBBaseBackend {
    ...
    createIndex(indexName, keyPath, options = {}) {
        let { multiEntry = false, upgradeCondition = null, unique = false, lmdbKeyEncoding = GenericValueEncoding } = options || {};

        if (this._db.connected) throw new Error('Cannot create index while connected');
        keyPath = keyPath || indexName;
        const index = new PersistentIndex(this, this._db, indexName, keyPath, multiEntry, unique, lmdbKeyEncoding);
